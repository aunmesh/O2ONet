import torch
import torch.nn
from model.nn_modules import ConvLSTM


class LinkFunction(torch.nn.Module):

    def __init__(self, link_def, config):
        ...
        self.__set_link(link_def, config)
        

    def forward(self, edge_features):
        return self.l_function(edge_features)

    def __set_link(self, link_def, config):
    ...
    def init_graph_conv(self):
        input_size = self.config['edge_feature_size']
        hidden_size = self.config['link_hidden_size']

        if self.config.get('link_relu', False):
            self.learn_modules.append(torch.nn.ReLU())
            self.learn_modules.append(torch.nn.Dropout())
        for _ in range(self.config['link_hidden_layers']-1):
    ...
    def init_graph_conv_lstm(self):
        input_size = self.config['edge_feature_size']
        hidden_size = self.config['link_hidden_size']
        hidden_layers = self.config['link_hidden_layers']
