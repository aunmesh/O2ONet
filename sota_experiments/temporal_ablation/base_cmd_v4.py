...
frame_nums = [9, 7, 5, 3, 1]
arg_list = frame_nums
gpus = [0,1,2,3,0]


base_command = f"python executor/main_strat_custom_log.py --config configs/gpnn_ablation.yaml --stratified {args.stratified} --num_frames "

MAX_RETRIES = 15

processes = {base_command + str(arg[0]) + " --gpu " + str(arg[1]): {'process': None, 'retries': 0} for arg in zip(arg_list, gpus)}
