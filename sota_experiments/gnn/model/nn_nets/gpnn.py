...
class GPNN(torch.nn.Module):
    ...
    def __init__(self, config):
        ...
        self.link_fun = LinkFunction('GraphConv', config).to(self.config['device']).double()
        self.sigmoid = torch.nn.Sigmoid().to(self.config['device']).double()
        self.message_fun = MessageFunction('linear_concat_relu', config).to(self.config['device']).double()
        self.update_fun = UpdateFunction('gru', config).to(self.config['device']).double()
    ...
    def forward(self, data_item):
        ...
        pred_adj_mat = torch.autograd.Variable( torch.zeros(batch_size, max_num_nodes, max_num_nodes) )
    ...
    def _load_link_fun(self, config):
        ...
        best_model_file = os.path.join(config['model_saving_path'], os.pardir, 'graph', 'model_best.pth')
