...
class mfurln(torch.nn.Module):
    ...
    def make_all_pairs(self, data_item):
        ...
        feature_matrix = torch.zeros(batch_size, 28,4206+77, device=self.config['device'])
        ...
        for b in range(batch_size):
            ...
            for i in range(curr_num_obj):
                for j in range(i+1,curr_num_obj):
                    ...
                    temp_edge_feat = data_item['relative_feature'][b,:, i,j].flatten()
                    
                    feature_matrix[b, curr_index, :4206] = temp_node_feat
                    feature_matrix[b, curr_index, 4206:] = temp_edge_feat
