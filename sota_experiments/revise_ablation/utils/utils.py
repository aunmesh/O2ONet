...
def permute_last_dim(tensor, config):
    """
    Permute the last dimension of the given tensor.

    Args:
    - tensor (torch.Tensor): The input tensor.

    Returns:
    - torch.Tensor: The tensor with its last dimension permuted.
    """

    # Get the size of the last dimension
    last_dim_size = tensor.size(-1)
    
    # Generate a permutation of indices for the last dimension
    perm_indices = torch.randperm(last_dim_size).to(config['device'])

    # print("IN PERMUTATION", tensor.device, perm_indices.device)

    # Permute the tensor using the generated indices
    return tensor.index_select(-1, perm_indices)
...
def process_data_for_fpass(data_item, config):
    ...
    # if ablation_key != 'None':
    #     data_item[ablation_key] = permute_last_dim(data_item[ablation_key], config)
