...
class hgat(torch.nn.Module):
    ...
    def make_classifier_inputs(self, obj_features, edge_features,
                               node_embeddings, triplet_embeddings, pairs):
        '''
        ...
        classifier_input = torch.empty(
            num_batches, num_pairs, 3*self.config['message_size'], device=self.config['device'])
        ...
        for b in range(num_batches):
            ...
            for i in range(num_pairs):
                ...
                temp_emb_1 = aggregate(emb0, emb1, 'concat')

                emb0, emb1 = obj_features[b, ind0], obj_features[b, ind1]
                temp_emb_2 = aggregate(emb0, emb1, 'concat')
                temp_emb = aggregate(temp_emb_1, temp_emb_2, 'mean')
                
                classifier_input[b, i, :2*self.config['message_size']] = temp_emb
                
                triplet_embedding = triplet_embeddings[b, i]
                
                emb0, emb1 = edge_features[b, ind0, ind1], edge_features[b, ind1, ind0]
                
                temp_emb0 = aggregate(emb0, emb1, 'mean')
                temp_emb = aggregate(temp_emb0, triplet_embedding, 'mean')
    ...
    def forward(self, data_item):
        ...
        obj_features = self.node_feature_resize(data_item['concatenated_node_features'])
        edge_features = self.edge_feature_resize(data_item['interaction_feature'])
