...
class GPNN(torch.nn.Module):
    ...
    def __init__(self, config):
        ...
        self.link_fun = LinkFunction('GraphConv', config).to(
            self.config['device'])
        self.sigmoid = torch.nn.Sigmoid().to(self.config['device'])
        self.message_fun = MessageFunction(
            'linear_concat_relu', config).to(self.config['device'])
    ...
    def make_classifier_inputs(self, node_embeddings, edge_features, pairs):
        '''
        ...
        for b in range(num_batches):
            ...
            for i in range(num_pairs):
                ...
                temp_agg = aggregate(emb0, emb1, 'concat')
                classifier_input[b, i, : 2 * self.config['message_size']] = temp_agg
                
                emb0, emb1 = edge_features[b, ind0, ind1], edge_features[b, ind1, ind0]
                temp_agg = aggregate(emb0, emb1, 'mean')
                classifier_input[b, i, 2 * self.config['message_size']: ] = temp_agg
    ...
    def forward(self, data_item):
        ...
        res_connection = final_embeddings + node_features.swapaxes(1, 2)
